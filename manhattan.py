...
class Manhattan:
    ...
    def trace(self, pos_list, destination):
        ...
        for i in reversed(path):
            self.mannhattan_path_cost += 1
            self.replace_manhattan(i.row, i.col)
        print("Total Manhattan Path Cost: {}".format(self.mannhattan_path_cost))

    def replace_manhattan(self, i_row, i_col):
    ...
    def calculateDistance(self, list, i_row, i_col, goal : Point):
        ...
        for index in range(len(self.list)):
            ...
            self.list[index].g = self.calculate_manhattan(self.list[index].original.row, self.list[index].original.col, self.list[index].current.row, self.list[index].current.col)
            print(self.list[index].g)
            self.list[index].h = self.calculate_manhattan(self.list[index].current.row, self.list[index].current.col, goal.row, goal.col)
    ...
    def calculate_manhattan(self, i_row, i_col, goal_row, goal_col):
        manhattan_distance = abs(i_row-goal_row) + abs(i_col-goal_col)
        return manhattan_distance
        #print(euclidean_distance)

    def search_manhattan(self, i_row, i_col, goal_row, goal_col):
        ...
        while(len(path_list) > 0):
            ...
            if self.isValid(north_row, col):
                ...
                if self.isGoal(north_row, col, goal):
                    ...
                    newH = float(self.calculate_manhattan(north_row, col, goal.row, goal.col))
                    newF = newG + newH
            ...
            if self.isValid(south_row, col):
                if self.isGoal(south_row, col, goal):
                    ...
                    newG = pos_list[row][col].g + 1.0
                    newH = float(self.calculate_manhattan(south_row, col, goal.row, goal.col))
                    newF = newG + newH
            ...
            if self.isValid(row, east_col):
                if self.isGoal(row, east_col, goal):
                    ...
                    newG = pos_list[row][col].g + 1.0
                    newH = float(self.calculate_manhattan(row, east_col, goal.row, goal.col))
                    newF = newG + newH
            ...
            if self.isValid(row, west_col):
                if self.isGoal(row, west_col, goal):
                    ...
                    newG = pos_list[row][col].g + 1.0
                    newH = float(self.calculate_manhattan(row, west_col, goal.row, goal.col))
                    newF = newG + newH
