...
class aStarEuclidean:
    def __init__(self, board, boardSize, barrierCoords):
        ...
        self.board = board
        self.aStarEuclidean_path_cost = 0
    ...
    def trace(self, pos_list, destination):
        ...
        for i in reversed(path):
            self.aStarEuclidean_path_cost += 1
            self.replace_aStarEuclidean(i.row, i.col)
        print("Total Euclidean Path Cost: {}".format(self.aStarEuclidean_path_cost))

    def replace_aStarEuclidean(self, i_row, i_col):
    ...
    def calculateDistance(self, list, i_row, i_col, goal : Point):
        ...
        for index in range(len(self.list)):
            ...
            self.list[index].g = self.calculate_aStarEuclidean(self.list[index].original.row, self.list[index].original.col, self.list[index].current.row, self.list[index].current.col)
            print(self.list[index].g)
            self.list[index].h = self.calculate_aStarEuclidean(self.list[index].current.row, self.list[index].current.col, goal.row, goal.col)
    ...
    def calculate_aStarEuclidean(self, i_row, i_col, goal_row, goal_col):
        aStarEuclidean_distance = math.sqrt((i_row-goal_row) ** 2 + (i_col-goal_col) ** 2)
        return aStarEuclidean_distance
        #print(euclidean_distance)

    def search_aStarEuclidean(self, i_row, i_col, goal_row, goal_col):
        ...
        while(len(path_list) > 0):
            ...
            if self.isValid(north_row, col):
                ...
                if self.isGoal(north_row, col, goal):
                    ...
                    newH = float(self.calculate_aStarEuclidean(north_row, col, goal.row, goal.col))
                    newF = newG + newH
            ...
            if self.isValid(south_row, col):
                if self.isGoal(south_row, col, goal):
                    ...
                    newG = pos_list[row][col].g + 1.0
                    newH = float(self.calculate_aStarEuclidean(south_row, col, goal.row, goal.col))
                    newF = newG + newH
            ...
            if self.isValid(row, east_col):
                if self.isGoal(row, east_col, goal):
                    ...
                    newG = pos_list[row][col].g + 1.0
                    newH = float(self.calculate_aStarEuclidean(row, east_col, goal.row, goal.col))
                    newF = newG + newH
            ...
            if self.isValid(row, west_col):
                if self.isGoal(row, west_col, goal):
                    ...
                    newG = pos_list[row][col].g + 1.0
                    newH = float(self.calculate_aStarEuclidean(row, west_col, goal.row, goal.col))
                    newF = newG + newH
